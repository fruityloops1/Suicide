...
static void InitDeko3Shaders(ImGui_ImplDeko3d_Data* bd)
{
    ...
    bd->codeMemBlock = dk::MemBlockMaker(device, CODEMEMSIZE)
                           .setFlags(DkMemBlockFlags_CpuUncached | DkMemBlockFlags_GpuCached | DkMemBlockFlags_Code)
                           .create();

    // load shaders
    u32 codeMemOffset = 0;
    codeMemOffset += loadShader(bd->vertexShader, "shaders/imgui_vsh.dksh",
        bd->codeMemBlock, codeMemOffset);
    codeMemOffset += loadShader(bd->fragmentShader, "shaders/imgui_fsh.dksh",
        bd->codeMemBlock, codeMemOffset);
...
static void InitDeko3dSwapchain(ImGui_ImplDeko3d_Data* bd)
{
    ...
    bd->depthMemBlock = dk::MemBlockMaker(device, align(align(depthLayout.getSize(), depthLayout.getAlignment()), (u32)DK_MEMBLOCK_ALIGNMENT))
    ...
    bd->fbMemBlock = dk::MemBlockMaker(device, FB_NUM * fbSize)
                         .setFlags(DkMemBlockFlags_GpuCached | DkMemBlockFlags_Image)
                         .create();
    ...
    for (int i = 0; i < FB_NUM; ++i) {
        bd->cmdbufMemBlock[i] = dk::MemBlockMaker(device, align(CMDMEMSIZE, DK_MEMBLOCK_ALIGNMENT))
...
static void InitDeko3dFontTexture(ImGui_ImplDeko3d_Data* bd)
{
    ...
    dk::UniqueMemBlock scratchMemBlock = dk::MemBlockMaker(device,
    ...
    dk::ImageLayoutMaker { device }
        ...
        .initialize(layout);
    bd->fontImageMemBlock = dk::MemBlockMaker {
        device, align(layout.getSize(), std::max(layout.getAlignment(), (u32)DK_MEMBLOCK_ALIGNMENT))
    }
                                .setFlags(DkMemBlockFlags_GpuCached | DkMemBlockFlags_Image)
                                .create();
...
static const u64 startTick = armGetSystemTick();
    ...
    int slot)
{
    ...
    vertUBO.proj = glm::orthoRH_ZO(0.0f, io.DisplaySize.x, io.DisplaySize.y, 0.0f,
        -1.0f, 1.0f);
    vertUBO.time = float(armGetSystemTick() - startTick) / armGetSystemTickFreq();
...
void ImGui_ImplDeko3d_RenderDrawData(ImDrawData* drawData)
{
    ...
    if (!bd->vtxMemBlock[slot] || bd->vtxMemBlock[slot].getSize() < totVtxSize) {
        ...
        bd->vtxMemBlock[slot] = dk::MemBlockMaker(bd->device,
    ...
    if (!bd->idxMemBlock[slot] || bd->idxMemBlock[slot].getSize() < totIdxSize) {
        ...
        bd->idxMemBlock[slot] = dk::MemBlockMaker(bd->device,
